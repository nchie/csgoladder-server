...
io.use(function(socket, next) {
  ...
  if(socket.cookies.access_token)
    try {
      socket.validtoken = jwt.verify(socket.cookies.access_token, config.jwtSecret);
    } catch(err) {
      //Invalid token
    }
...
io.on('connection', function(socket){
  ...
  {
    console.log("User " + socket.validtoken.user.id + " joined room \"user:" + socket.validtoken.user.id + "\"")
    socket.join("user:" + socket.validtoken.user.id);
    socket.join("users");
